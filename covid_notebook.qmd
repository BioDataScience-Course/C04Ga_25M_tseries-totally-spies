...
On peut voir dans ce graphique que nous avons une très grande augmentation du temps passée à la maison en 2020, cela correspond au moment du premier confinement, on voit aussi une seconde augmentation en 2021 ( entre la 30ème jours et 80èmejours), qui correpond confinement puis on assiste à une baisse progressive qui correspond au déconfinement progressif.

On peut aussi noter que en 2022 lors du retour à la "normale" le temps passée à la maison est légèrement plus haut que ce qu'il était au tout début 2020 avant le covid.

Essayons maintenant de reproduire notre analyse, en prenant en compte seulement les jours de la semaine ( qui sont en théorie les jours où en temps normal les gens sont moins souvent chez eux). Nous faisons aussi une moyenne pour avoir une valeur pour chaque semaine de l'année.
...
Notre analyse est la même que précédemment, on voit très bien les effets des différents confinements comme expliqué précedemment.
...
-\> Après reflexion, faire un lissage en utilisant la méthode "median" comme fait lors de la prèmière décomposition, revient aussi à ne pas prendre en compte les données du weekend ( vu que la valeur médiane avec order = 8 revient à prendre une valeur de la semaine à chaque fois) Nos 2 analyses sont donc très similaire, nous n'en garderons qu'une dans le rapport final.
...
L'acf montre qu'il existe potentiellement un cycle de fréquence 0,02 (correspondant à un cycle tous les 7jours plus ou moins.
...
plot(tsjob)
# Aggrégation par moyenne des valeurs par semaine
covid_tsjob2 <- aggregate(tsjob, 52, FUN = collapse::fmean, ts.eps = 1/52)
plot(covid_tsjob2, main = "Déplacement au travail du Lundi au dimanche")
job_ttsem <- tsd(covid_tsjob2, method = "average", order = 5)
plot(job_ttsem, col = 1:3)
```
...
abline(v = c(5, 12), lty = 2, col = "green")
...
Besoin de stationnariser avant de faire une analyse spectrale

```{r}
job_stl <- stl(tsjob, s.window=8)
seasonal <- job_stl$time.series[, "seasonal"]

spectrum(seasonal, spans = c(3,5))
```

Cycle signifiactif à une fréquence de 52. 365/52 = +/- 7j. Je ne sais pa si on doit prendre en compte les valeurs du début.

### Série 3 : Déplacements jusqu'au commerces et lieux de loisirs en fonction de l'évolution du Covid-19

Intéressons nous a la mobilité pour les lieux de commerces et loisirs.

```{r activites}
activite_ts <- ts(covid1$activité, start = c(2020,46), frequency = 365)
plot(activite_ts, main = "Mobilité pour les commerces et loisirs")
```

Un filtrage par EVF est réalisé pour débruiter la séries
...
Il y a une diminution drastiques des déplacements vers les commerces et activités de loisirs au début de 2020. Cette diminution est suivie d'une forte augmentation des déplacements vers les commerces et loisirs 3 mois plus tard. Un palier est observé ensuite. Il y a une légère rechute fin 2020 avec une augmentation progressive de mobilité pour les commerces et loisirs au début 2021. Cette augmentation se stabilise durant l'année 2021 et fluctuent très peu au cours de l'année 2022. On remarque que les déplacements vers les commerces et loisirs est légèrement supérieur a partir de 2021 par rapport au première mesures de 2020.
...
Un filtrage de méthode des médianes a été réalisé pour tester un filtrage différent et pour in fine choisir la meilleur méthode.
...
plot(activite_ts, 
  main = "Mobilité des citoyens vers les commercerces et loisirs",
     xlab = "Année",
...
Ce graphique permet de visualiser le peu de différences entre les méthodes de filtrages. Le filtrage EVF sera choisi pour le rapport car la méthode permet d'avoir plus d'infos que la méthode des médianes qui segmente la série en calculant les médianes d'une fenetre mobiles.

Pour la suite de l'analyse, les jours de la semaine et les weekends ont été séparés en deux séries temporelles distinctes. En effet, une comparaison des déplacements vers les commerces et les loisirs entre la semaine et le week-end sera réalisée.
...
Un filtrage EVF est réalisé car il correspond bien a la série temporelle.

```{r}
covidaw_ts2_mb <- tsd(covidaw_ts2, method = "evf", lag = 5 , axes = c(1,2))
plot(covidaw_ts2_mb, col = 1:3)
```
...
# Les données dela semaine sont remplacées par des valeurs manquantes
...
plot(covidawe_ts2_mb, col = 1:3)
```

Les séries filtrées sont extraites et comparer sur un meme graphique avec la série brutes.
...
plot(activite_ts,
     main = " Moilité vers les commerces et loisirs pendant le Covid",
     xlab = "Année",
...
En début 2020, les déplacements vers les commerces et lieux de loisirs la semaine et le week-end sont très proches et diminuent fortement comme vu précedement. Les traits en pointillé vert correspondent a la premiere période de confinement. Il y a ensuite une augmentation 3 mois plus tard avec également peu de différence entre les jours de la semaine et le week-end. La fin d'année de 2021est marquée par une diminution de fréquentations des commerces et loisirs. Les traits en pointillérouges correspondent a la date du second confinement. Pendant l'année 2021, les déplacements vers les commerces et les loisirs est plus faibles le week-end que la semaine. Cela reste similaire le restant de l'année 2021. A partir de 2022 la mobilité vers les commerces et loisirs le week-end est légèrement plus élevé que la semaine. La fréquentation des commerces et lieux de loisirs est légerement plus élevée a partir de la moitié de l'année 2021 par rapport au début de la série en 2020.
...
Le premier confinement, du 18 mars au 4 mai 2020, s’accompagne de fortes restrictions sur les loisirs et le sport, de la généralisation du télétravail et de la fermeture des écoles et universités [@RTBF]. Le second confinement débute le 2 novembre 2020, dans le contexte d’une deuxième vague apparue après le retour des vacances scolaires. Son assouplissement intervient progressivement à partir de mars 2021, environ trois mois après le début de la campagne de vaccination (5 janvier 2021), avec la mise en place d’un confinement renforcé visant à limiter la circulation du virus [@vrtnws]. En conclusion, nos résultats correspondent bien à ce qu’on connaît de la période et permettent de mieux comprendre les changements de mobilité liés à la pandémie. Ils offrent une bonne base de discussion et nous intégrerons les analyses les plus pertinentes dans notre rapport final.
